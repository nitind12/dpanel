...
class Upcoming extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('My_model_upcoming', 'mmnws');
    ...
    function edit_upcoming($id__) {
        ...
        $data['news_d'] = $this->mmnws->get_all_news_deactive();
        $data['menu'] = $this->mm->get_menu($this->session->userdata('stss_'));
        $data['edit_page_heading'] = 'Update Upcoming Events';
        $data['edit_page'] = "upcoming/editupcoming";
        $data['view1'] = "upcoming/viewupcoming_active";
        $data['view2'] = "upcoming/viewupcoming_deactive";
        $data['wallpaper_'] = '';
    ...
    function updateUpcoming($id__) {
        $res_ = $this->mmnws->updateNews_($id__);
        $this->session->set_flashdata('feed_msg_', $res_['msg_']);
        redirect('upcoming/edit_upcoming/' . $id__);
    }

    function active_deactive_upcoming($id_, $status_) {
    ...
    function feedUpcoming() {
        $res_ = $this->mmnws->feedNews_();
        $this->session->set_flashdata('feed_msg_', $res_['msg_']);
        redirect('upcoming');
    }

    function delete_upcoming_events($id_) {
    ...
    function checkMenuAuthentication(){
        if($this->mm->my_menu($this->session->userdata('stss_'), 'upcoming') == false){
            redirect('dashboard');
        }
    }
}