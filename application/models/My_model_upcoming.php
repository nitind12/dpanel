...
class My_model_upcoming extends CI_Model {
	...
	function feedNews_() {
        $data = array(
            ...
            'UPCOMING_EVENT' => $this->input->post('txtNews'),
            'PATH_ATTACH' => 'x',
            'DATE_' => date('d/m/Y'),
        ...
        $config = array(
            'upload_path' => './_assets_/upcomingevents',
            'overwrite' => TRUE,
    ...
    function get_latest_news($limit_) {
        ...
        $query = $this->db->get('upcoming', 0, $limit_);
        return $query->result();
    }
    ...
    function delete_news_events($id_) {
        ...
        $this->db->where('ID', $id_);
        $bool_ = $this->db->delete('upcoming');
        if ($bool_ == TRUE) {
            if ($file__ != 'x') {
                $full_path_ = FCPATH . '_assets_/upcomingevents/' . $file__;
                @unlink($full_path_);
            }
        }
        return $bool_;
